...
<script type="text/javascript">
    window.onload=function(){
        window.print();
        <?php if ($viewAudit) { ?>
            window.location.href = "<?php echo $this->createUrl('printMemberAudit',['id'=>$model->id])?>";
        <?php } else { ?>
...
<div style="-webkit-print-color-adjust:exact;border:1px black solid; margin: 5px; padding:10px; width: 7in; height: 4.5in; font-family: Arial">
  ...
  <div style = "float: left; direction: rtl"><?php echo Helpers::receiptNumber($memberTransaction) ?></div>&nbsp;&nbsp;&nbsp;
	...
	<br>
	<div style = "text-align:center; width:7in;font-size: 25px; font-family: FarumaWeb, Faruma">ފައިސާގެ ރަސީދު</div><br>
    <?php
    ...
    <table style="font-family: FarumaWeb,Faruma; font-size: 14px">
        ...
        <tr>
            <td style="direction: rtl;text-align: right; background: #ddd;">
                <?php echo '<span style="font-family:Arial;">'.Helpers::currency($memberTransaction->amount).'</span>' ?> ރުފިޔާ
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(މިހާތަނަށް ޖުމްލަ ޖަމާކުރެވުނު ޢަދަދު: <?='<span style="font-family:Arial;">'.Helpers::currency($memberTransaction->balance).'</span>'?> ރުފިޔާ)
            </td>
            <td style="width: 2in; text-align: right">:ޖަމާކުރި ޢަދަދު</td>
        </tr>
        <tr>
            <td style="width: 5in; direction: rtl;text-align: right; background: #ddd;">
                <span style="width:100%; ; direction: rtl"><?php echo '('.CurrencyText::parse($memberTransaction->amount,'dv').')' ?>
                </span>
            </td>
        ...
        <tr style="color:#666">
            ...
            <td style="width: 2.5in; text-align: right; direction: rtl">
                <span style="width:100%;">&nbsp;<?php echo $memberTransaction->user->person->full_name_dhivehi
                      . ' (އައިޑީ ' . $memberTransaction->user->id . ') '?>&nbsp;</span>
            </td>

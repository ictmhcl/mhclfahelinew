...
$childrenNames = '';
foreach($transaction->ageega->ageegaChildrens as $ageegaChildren)
  $childrenNames .= ((!empty($childrenNames)?'، ':'') .
    $ageegaChildren->full_name_dhivehi);

if (!$view) { ?>
  <script type="text/javascript">
    window.onload = function () {
      window.print();
      window.location.href = "<?php echo $this->createUrl('ageegaPayment')?>";
    };
  </script>
<?php } ?>
<div
  ...
  <div
    style="float: left; direction: rtl"><?php echo Helpers::ageegaReceiptNumber($transaction)
    ?></div>
  &nbsp;&nbsp;&nbsp;
  ...
  <table style="font-family: FarumaWeb,Faruma; font-size: 14px">
    <tr>
      <td style="width: 1.5in; text-align: right">
        <span
          style="font-family: Arial;width:100%; border-bottom: 1px dotted grey"></span>
      </td>
      <td style="width: 1in; text-align: right; direction: rtl">&nbsp;
      </td>
      <td style="width: 2.5in; text-align: right; direction: rtl">
                <span style="width:100%; border-bottom: 1px dotted
                grey">
                  <?php echo $transaction->ageega->person->full_name_dhivehi .
                    '<span style="font-family:Arial"> ('
                    . $transaction->ageega->person->id_no . ')</span>' ?></span>
      </td>
      <td style="width: 2in; text-align: right">:ހުށަހެޅި ފަރާތް</td>
    </tr>
    <tr>
      <td style="width: 5in; text-align: right" colspan="3">
        <span
          style="width:100%; border-bottom: 1px dotted grey"><?php echo $transaction->ageega->phone_number ?></span>
      </td>
      <td style="width: 2in; text-align: right">:ފޯނު ނަންބަރު</td>
    </tr>
  </table>
  <hr>
  <table style="font-family: FarumaWeb,Faruma; font-size: 14px">
    <tr>
      ...
      <td
        ...
        <span
          style="width:100%;  ">&nbsp;<?php echo $transaction->description_dhivehi . '
          (ކުދިން: ' . $childrenNames . ')'
          ?></span>
      </td>
  ...
  <table style="font-family: FarumaWeb,Faruma; font-size: 12px">
    <tr style="color:#666">
      ...
      <td style="width: 2.5in; text-align: right; direction: rtl">
                <span style="width:100%;">&nbsp;<?php echo $transaction->user->person->full_name_dhivehi
                    . ' (އައިޑީ ' . $transaction->user->person->id_no . ')
                    '?>&nbsp;</span>
      </td>
      <td style="width: 1.5in; text-align: right; direction: rtl">ފައިސާ ބަލައިގަތީ:</td>
    </tr>
  </table>

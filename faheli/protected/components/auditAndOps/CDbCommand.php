...
class CDbCommand extends CComponent {
   ...
   * for more details.
   */
  public function __construct(CDbConnection $connection, $query = null, $auditRecord = null) { //with Audit Logging
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord = !is_null($auditRecord) ? $auditRecord : null;
    } else {
  ...
  public function prepare() {
    if ($this->_statement == null) {
      try {
        ...
        throw new CDbException(Yii::t('yii', 'CDbCommand failed to prepare the SQL statement: {error}', ['{error}' => $e->getMessage()]), (int) $e->getCode(), $errorInfo);
      }
    }
  }
   ...
   */
  public function execute($params = []) { //with Audit Logging
    ...
    try {
      ...
      if ($this->auditLogging) {
        if (is_null($this->auditRecord))
          $this->auditRecord = new Audits;

        $this->auditRecord->sql_command = $this->getText() . $par;
        $this->auditRecord->command_begin_time = microtime(true);
      }
      //<--Audit Logs
      ...
      //Audit Logs
      if ($this->auditLogging && $n > 0) {
        $this->auditRecord->command_end_time = microtime(true);
        $this->auditRecord->rows_affected = $n;
        $this->auditRecord->sql_success = 1;
        $this->auditRecord->save(false);
      }
      //<-- Audit Logs
      ...
      Yii::log(Yii::t('yii', 'CDbCommand::execute() failed: {error}. The SQL statement executed was: {sql}.', ['{error}' => $message, '{sql}' => $this->getText() . $par]), CLogger::LEVEL_ERROR, 'system.db.CDbCommand');
      if (YII_DEBUG)
      ...
      if ($this->auditLogging) {
        $this->auditRecord->command_end_time = microtime(true);
        $this->auditRecord->rows_affected = 0;
        $this->auditRecord->sql_success = 0;
        $this->auditRecord->save(false);
        $this->auditRecord = null;
      }
      //<-- Audit Logs
  ...
  private function queryInternal($method, $mode, $params = []) {
    ...
    try {
      ...
      if ($method === '')
        ...
        $mode = (array) $mode;
        $result = call_user_func_array([$this->_statement, $method], $mode);
      ...
      $errorInfo = $e instanceof PDOException ? $e->errorInfo : null;
      $message = $e->getMessage();
      Yii::log(Yii::t('yii', 'CDbCommand::{method}() failed: {error}. The SQL statement executed was: {sql}.', ['{method}' => $method, '{error}' => $message, '{sql}' => $this->getText() . $par]), CLogger::LEVEL_ERROR, 'system.db.CDbCommand');
      if (YII_DEBUG)
        $message .= '. The SQL statement executed was: ' . $this->getText() . $par;
      throw new CDbException(Yii::t('yii', 'CDbCommand failed to execute the SQL statement: {error}', ['{error}' => $message]), (int) $e->getCode(), $errorInfo);
    }
   ...
   * @since 1.1.6
   */
  public function insert($table, $columns) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    //<-- Audit Logs
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (inserted into table)';
      $this->auditRecord->crud_type = Audits::CRUD_CREATE;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function update($table, $columns, $conditions = '', $params = []) { //with Audit Logging
    // Audit Logs - not allowed on audit tables
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    //<-- Audit Logs
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (updated in table)';
      $this->auditRecord->crud_type = Audits::CRUD_UPDATE;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function delete($table, $conditions = '', $params = []) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (deleted from table)';
      $this->auditRecord->crud_type = Audits::CRUD_DELETE;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function createTable($table, $columns, $options = null) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (new table created)';
      $this->auditRecord->crud_type = Audits::CREATE_TABLE;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function renameTable($table, $newName) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (new table name)';
      $this->auditRecord->crud_type = Audits::RENAME_TABLE;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function dropTable($table) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (dropped table)';
      $this->auditRecord->crud_type = Audits::DROP_TABLE;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function truncateTable($table) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (truncated table)';
      $this->auditRecord->crud_type = Audits::TRUNCATE_TABLE;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function addColumn($table, $column, $type) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) |' . $column . ' (added column)';
      $this->auditRecord->crud_type = Audits::ADD_COLUMN;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function dropColumn($table, $column) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) |' . $column . ' (dropped column)';
      $this->auditRecord->crud_type = Audits::DROP_COLUMN;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function renameColumn($table, $name, $newName) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) |' . $newName . ' (new column name)';
      $this->auditRecord->crud_type = Audits::RENAME_COLUMN;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function alterColumn($table, $column, $type) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) |' . $column . ' (altered column)';
      $this->auditRecord->crud_type = Audits::ALTER_COLUMN;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function addForeignKey($name, $table, $columns, $refTable, $refColumns, $delete = null, $update = null) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) | ' . $refTable . ' (foreign key created to table)';
      $this->auditRecord->crud_type = Audits::ADD_FK;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function dropForeignKey($name, $table) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) |' . $name . ' (dropped key)';
      $this->auditRecord->crud_type = Audits::DROP_FK;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function createIndex($name, $table, $column, $unique = false) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) |' . $name . ' (new Index created)';
      $this->auditRecord->crud_type = Audits::CREATE_INDEX;
    }
    //<-- Audit Logs
   ...
   * @since 1.1.6
   */
  public function dropIndex($name, $table) { //with Audit Logging
    // Audit Logs - not allowed on audit table
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog)
      if (in_array($table, Yii::app()->audit->auditTables))
        return null;
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      $this->auditRecord->active_record_class = $table . ' (table) |' . $name . ' (index dropped)';
      $this->auditRecord->crud_type = Audits::DROP_INDEX;
    }
    //<-- Audit Logs
  ...
  private function processConditions($conditions) {
    ...
    throw new CDbException(Yii::t('yii', 'Unknown operator "{operator}".', ['{operator}' => $operator]));
  }

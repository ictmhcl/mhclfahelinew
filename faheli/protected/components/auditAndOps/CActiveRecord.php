...
abstract class CActiveRecord extends CModel
{
	...
	private static $_models= [];			// class name => model

	private $_md;								// meta data
	private $_new=false;						// whether this instance is new or not
	private $_attributes= [];				// attribute name => attribute value
	private $_related= [];					// attribute name => related objects
	...
	public function getRelated($name,$refresh=false,$params= [])
	{
		...
		if($this->getIsNewRecord() && !$refresh && ($relation instanceof CHasOneRelation || $relation instanceof CHasManyRelation))
			return $relation instanceof CHasOneRelation ? null : [];

		if($params!== []) // dynamic query
		{
	 ...
	 */
	public function insert($attributes=null)  // with Audit Logging
	{
      ...
      //Operation Logs
      if (!Yii::app()->user->isGuest && isset(Yii::app()->audit) && Yii::app()->audit->opLog && 
        (!in_array($this->tableName(), Yii::app()->audit->auditTables))) {
          Yii::app()->audit->operationLog($this);
      }
      //<-- Operation Logs
      ...
      if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
        if (!in_array($this->tableName(), Yii::app()->audit->auditTables)) {
          $command->auditLogging = true;
          $command->auditRecord = new Audits;
          $command->auditRecord->record_data = CJSON::encode($this->attributes);
          $command->auditRecord->crud_type = Audits::CRUD_CREATE;
          $command->auditRecord->active_record_class = get_class($this);
          $command->auditRecord->transaction_id =
                  is_null($this->dbConnection->getCurrentTransaction())?
                  null:$this->dbConnection->getCurrentTransaction()->logId;
        } else {
          $command->auditLogging = false;
        }
        $this->dbConnection->enableParamLogging = true;
      }
      //<-- Audit Logs
        ...
        if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
          if (!in_array($this->tableName(), Yii::app()->audit->auditTables)) {
            ...
            if (is_array($pk))
              $command->auditRecord->multi_record_ids = CJSON::encode($pk);
            else
              $command->auditRecord->record_id = $pk;

            $command->auditRecord->save(false);
            $command->auditRecord = null;
          }
        }
        //<-- Audit Logs
        
        //XML Audit Logs
        if (!is_null($this->dbConnection->getCurrentTransaction()) &&
                (Yii::app()->audit->xmlLog) && (Yii::app()->audit->collectChildren))
          if ($this!==Yii::app()->audit->xmlAuditLog->getMainItem())
            $this->dbConnection->getCurrentTransaction()->addAuditChildren($this);
        //<-- XML Audit Logs
	...
	public function applyScopes(&$criteria)
	{
		if(!empty($criteria->scopes))
		{
			...
			foreach((array)$criteria->scopes as $k=>$v)
			{
				...
				call_user_func_array([$this,$scope],(array)$params);
			}
		}
	 ...
	 */
	public function updateByPk($pk,$attributes,$condition='',$params= []) //with Audit Logging
	{
    ...
    if (!Yii::app()->user->isGuest && isset(Yii::app()->audit) && Yii::app()->audit->opLog && 
      (!in_array($this->tableName(), Yii::app()->audit->auditTables))) {
        $tblModel = $this->findByPk($pk);
        
        $opLogId = Yii::app()->audit->operationLog($tblModel);
        $attributes['operation_log_id'] = $opLogId;
        $table = $tblModel->getTableSchema();
    }
    //<-- Operation Logs
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      if (!in_array($this->tableName(), Yii::app()->audit->auditTables)) {
        ...
        if (is_array($this->getPrimaryKey()))
          $command->auditRecord->multi_record_ids = CJSON::encode($this->getPrimaryKey());
        else
          $command->auditRecord->record_id = $this->getPrimaryKey();

        $command->auditRecord->record_data = CJSON::encode($this->attributes);
        $command->auditRecord->crud_type = Audits::CRUD_UPDATE;
        $command->auditRecord->active_record_class = get_class($this);
        $command->auditRecord->transaction_id =
                is_null($this->dbConnection->getCurrentTransaction())?
                null:$this->dbConnection->getCurrentTransaction()->logId;
	 ...
	 * @return integer the number of rows being updated
	 */
	public function updateAll($attributes,$condition='',$params= []) //with Audit Logging
	{
    ...
    if (!Yii::app()->user->isGuest && isset(Yii::app()->audit) && Yii::app()->audit->opLog && 
      (!in_array($this->tableName(), Yii::app()->audit->auditTables))) {
      $tblModels = $this->findAll($condition,$params);
      
      foreach ($tblModels as $tblModel) {
        $opLogId = Yii::app()->audit->operationLog($tblModel);
        $criteria = Yii::app()->db->commandBuilder->createPkCriteria($tblModel->tableName(),
                $tblModel->primaryKey);
        $updateOpLogIdSql = "Update ".$this->tableName()." SET operation_log_id = ".$opLogId.
                " WHERE ".$criteria->condition;
        Yii::app()->db->createCommand($updateOpLogIdSql)->execute();
      }
    }
    //<-- Operation Logs
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      ...
      if (!in_array($this->tableName(), Yii::app()->audit->auditTables)) {
        ...
        $effectedModels = self::model()->findAll($criteria);
        if (sizeof($effectedModels)>1) {
          $effectedRecords = [];
          $effectedPks = [];
          foreach($effectedModels as $effectedModel) {
            $singleRecord = $effectedModel->attributes;
            $effectedRecords[]=$singleRecord;
            $effectedPks[] = $effectedModel->getPrimaryKey();
          }
          $command->auditRecord->record_data = CJSON::encode($effectedRecords);
          $command->auditRecord->multi_record_ids = CJSON::encode($effectedPks);
        }
        if (sizeof($effectedModels)==1) {
          if (is_array($effectedModels[0]->getPrimaryKey()))
            $command->auditRecord->multi_record_ids = 
                CJSON::encode($effectedModels[0]->getPrimaryKey());
          else
            $command->auditRecord->record_id = $effectedModels[0]->getPrimaryKey();

          $command->auditRecord->record_data = CJSON::encode($effectedModels[0]->attributes);
        }

        $command->auditRecord->crud_type = Audits::CRUD_UPDATE_ALL;
        $command->auditRecord->active_record_class = get_class($this);
        $command->auditRecord->transaction_id =
                is_null($this->dbConnection->getCurrentTransaction())?
                null:$this->dbConnection->getCurrentTransaction()->logId;
    ...
    if (!is_null($this->dbConnection->getCurrentTransaction()) &&
            (Yii::app()->audit->xmlLog) && (Yii::app()->audit->collectChildren))
      if (($this!==Yii::app()->audit->xmlAuditLog->getMainItem()) && (isset($effectedModels)))
        $this->dbConnection->getCurrentTransaction()->addAuditChildren($effectedModels);
    //<-- XML Audit Logs
	 ...
	 */
	public function deleteByPk($pk,$condition='',$params= []) //with Audit Logging
	{
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      if (!in_array($this->tableName(), Yii::app()->audit->auditTables)) {
        ...
        $command->auditRecord->crud_type = Audits::CRUD_DELETE_BYPK;
        $command->auditRecord->active_record_class = get_class($this);
        $command->auditRecord->transaction_id =
                is_null($this->dbConnection->getCurrentTransaction())?
                null:$this->dbConnection->getCurrentTransaction()->logId;
	 ...
	 * @return integer the number of rows deleted
	 */
	public function deleteAll($condition='',$params= []) //with Audit Logging
	{
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      if (!in_array($this->tableName(), Yii::app()->audit->auditTables)) {
        ...
        if (sizeof($effectedModels)==1) {
          if (is_array($effectedModels[0]->getPrimaryKey()))
            $command->auditRecord->multi_record_ids = CJSON::encode($effectedModels[0]->getPrimaryKey());
          else
            $command->auditRecord->record_id = $effectedModels[0]->getPrimaryKey();
        ...
        $command->auditRecord->crud_type = Audits::CRUD_DELETE_ALL;
        $command->auditRecord->active_record_class = get_class($this);
        $command->auditRecord->transaction_id =
                is_null($this->dbConnection->getCurrentTransaction())?
                null:$this->dbConnection->getCurrentTransaction()->logId;
	 ...
	 */
	public function deleteAllByAttributes($attributes,$condition='',$params= []) //with Audit Logging
	{
    ...
    if (isset(Yii::app()->audit) && Yii::app()->audit->rawLog) {
      if (!in_array($this->tableName(), Yii::app()->audit->auditTables)) {
        ...
        $command->auditRecord->crud_type = Audits::CRUD_DELETE_ALL_BYATTR;
        $command->auditRecord->active_record_class = get_class($this);
        $command->auditRecord->transaction_id =
                is_null($this->dbConnection->getCurrentTransaction())?
                null:$this->dbConnection->getCurrentTransaction()->logId;
		...
		if($attributes!==false)
		{
			...
			foreach($attributes as $name=>$value)
			{
				if(property_exists($record,$name))
					...
					if ($md->columns[$name]->dbType == "bit(1)")
						$record->_attributes[$name]=(int) ($value && $value!=="\0");
					else
...
class CActiveRecordMetaData
{
	...
	public function addRelation($name,$config)
	{
		if(isset($config[0],$config[1],$config[2]))  // relation class, AR class, FK
			...
			throw new CDbException(Yii::t('yii','Active record "{class}" has an invalid configuration for relation "{relation}". It must specify the relation type, the related active record class and the foreign key.', ['{class}'=>get_class($this->_model),'{relation}'=>$name]));
	}

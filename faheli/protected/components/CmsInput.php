...
class CmsInput extends CApplicationComponent
{
    ...
    public function init()
    {
        ...
        parent::init();
        Yii::app()->attachEventHandler('onBeginRequest', [$this, 'cleanGlobals']);
    }
    ...
    protected function cleanGlobals()
    {
        ...
        if($this->getCleanPost()===true && $this->cleanPostCompleted===false && !empty($_POST))
        {
            ...
            if(defined('YII_DEBUG')&&YII_DEBUG)
                Yii::log(Yii::t('security', 'Global {global} array cleaned using {method} method.', ['{global}'=>'$_POST', '{method}'=>__CLASS__.'::'.$cleanMethod]));
        }
        if($this->getCleanGet()===true && $this->cleanGetCompleted===false && !empty($_GET))
        {
            ...
            if(defined('YII_DEBUG')&&YII_DEBUG)
                Yii::log(Yii::t('security', 'Global {global} array cleaned using {method} method.', ['{global}'=>'$_GET', '{method}'=>__CLASS__.'::'.$cleanMethod]));
        }
    }

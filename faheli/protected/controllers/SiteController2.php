    ...
    return [
        ['allow', // allow all users to perform 'index' and 'view' actions
            'actions' => ['login','verify','error','code','logout',
                          ...
                          'registrationSubmitted', 'inaugurate'],
            'users' => ['*'],
        ],
  ...
  public function actionVerify($animate = false) {
    ...
    if (isset($_POST['PhoneVerifyForm'])) {
      ...
      if ($model->validate() && $model->sendVerificationCode()) {
        Yii::app()->user->setFlash('info', 'ވެރިފިކޭޝަން ކޯޑު މޯބައިލަށް ފޮނުވިއްޖެ. ލިބިނުލައްވާނަމަ އިރުކޮޅަކު މަޑުކުރައްވާ');
    ...
    if (!empty($_POST['OnlineRegistrationForm'])) {
        ...
        if ($error > 0) {
          ...
          if (empty($fileName)) {
            ...
            $onlineForm->addError($fileAttribute, $onlineForm->getAttributeLabel($fileAttribute)
              ...
              . ') was not accepted! File may be larger than server limit.');
          }
        }
          ...
          if (!empty($onlineForm[$fileAttribute])
            ...
            Yii::app()->user->setFlash('error', 'There has been an internal
                 error! Could not replace existing file!');
          }
      ...
      try {
        ...
        if ($onlineForm->save()) {
          ...
          Yii::app()->user->setFlash('success', $onlineForm->full_name_dhivehi . ' ގެ ރަޖިސްޓޭޝަން ފޯމު ލިބިއްޖެ.
          ...
          Helpers::textMessage($onlineForm->phone_number_1, $onlineForm->full_name_english
            . ' ge online registration form MHCL ah libijje. Registration check'
          ...
          $this->redirect(['registrationSubmitted', 'id' => $onlineForm->id]);
        }
        if ($onlineForm->hasErrors('id_card_copy') == false)
          $onlineForm->addError('main', 'އައި.ޑީ ކާޑު ކޮޕީ އަލުން އެޓެޗް ކުރައްވާ');
      } catch (CException $ex) {
        ErrorLog::exceptionLog($ex);
        Yii::app()->user->setFlash('error', 'ދިމާވި މައްސަލައކާއި ގުޅިގެން
...
EOT;
  ...
  public function actionRegistrationSubmitted($id) {
    $onlineForm = OnlineRegistrationForm::model()->findByPk($id);
    if (empty($onlineForm)) {
  ...
  private function sendOnlineRegistrationReceived(OnlineRegistrationForm $onlineForm) {
    ...
    try {
      $mail->isHTML(true);
      $mail->SetFrom('admin@mhcl.mv', Helpers::config('organisationName'));
      ...
      mail('admin@mhcl.mv', 'Online Portal Regisration Received', $onlineForm->full_name_english
        . ' has requested for Online Portal
      Registration. Please verify the registration.', "From: admin@mhclonline.com");
